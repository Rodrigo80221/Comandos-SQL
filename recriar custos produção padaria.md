...
-- DELETAR OS REGISTROS DE CADASTRO DE PRODUTOS NA TABELA CUSTO HISTÓRICO
	...
	and month(DtInsercao) >= 1 and AlteraCusto = 1 
	and CodProduto in (select CodItem from ProducaoPadariaProdutos)
...
GO
	...
	DELETE from CustoHistorico where CodNota IN (
		...
		where year(DT_ENTRADA) = 2022 and month(DT_ENTRADA) >= 1 
		and nfp.CD_PRODUTO in (select CodItem from ProducaoPadariaProdutos)
	)
...
FOR 
	...
	(
		...
		where 
		year(DtFormula) = 2022 and month(DtFormula) >= 1 
...
DELETE from CustoHistorico where year(DtCusto) = 2022 and month(DtCusto) >= 1
