...
SELECT T1.codLoja , T1.codProduto, (select DescricaoReduzida from produtos where codigo = t1.codProduto) [Descrição],  T1.valorproduto, T1.ValorNoPdv, T2.VALOR_ALTERADO [PRECO ANTIGO], T2.DATA_ALTERACAO , (SELECT CD_ASSOCIADO FROM PRODUTOS WHERE CODIGO = T1.codProduto) CD_ASSOCIADO  FROM 
(
	SELECT PL.codLoja,PL.codProduto,PL.ValorNoPdv, pl.valorProduto FROM ProdutoLojas PL
	WHERE PL.codProduto in (SELECT CODIGO FROM PRODUTOS WHERE SUBSTRING(CONFIG,1,1) = '1') AND PL.valorProduto <> 0
) T1
INNER JOIN 
(
...
WHERE t1.codLoja = 1 and ABS(T1.ValorNoPdv - T2.VALOR_ALTERADO ) >= (T1.ValorNoPdv * (@DIFERENCA/100))

ORDER BY ABS(T1.ValorNoPdv - T2.VALOR_ALTERADO )  DESC
